...
std::string GetApiFootballRound(PGconn* pg, ELeague league, int week, int team1Id)
{
	...
	else if (league == ELeague::CopaDelRey || league == ELeague::CoppaItalia || league == ELeague::DFBPokal || league == ELeague::CoupeDeFrance || league == ELeague::FACup)
	{
		if (week == 1)
		{
			round = "Quarter-finals";
...
void ProcessEFootballResults(PGconn* pg, 
	int matchId, 
	const std::string& matchStatus, int team1Id, int team2Id, 
	int team1Goals, 
	int team2Goals,
	int team1GoalsAET,
	int team2GoalsAET,
	...
	int isSpecial)
{
	...
	for (int i = 0; i < nrows; ++i)
	{
		...
		if (teamId == team1Id) 
		{
			if (matchStatus == "PEN")
			{
				points = team1GoalsPEN > team2GoalsPEN ? 2 : -1;
			}
			else if (matchStatus == "AET")
			{
				points = (team1GoalsAET - team1Goals) > (team2GoalsAET - team2Goals) ? 2 : -1;
			}
			else 
			{
		...
		else if (teamId == team2Id)
		{
			...
			{
				points = (team2GoalsAET - team2Goals) > (team1GoalsAET - team1Goals) ? 2 : -1;
			}
			else
			{
		...
		if (r.status_code == 200)
		{
			...
			if (status == "FT" || status == "AET" || status == "PEN")
			{
				...
				else if (status == "AET")
				{
					...
					team1GoalsAET = team1Goals;
					team2GoalsAET = team2Goals;
				...
				//if (status == "FT")
				{
					...
					ProcessEFootballResults(pg, id, 
						status, team1.Id, team2.Id, 
						team1Goals90, team2Goals90,
						team1GoalsAET, team2GoalsAET,
...
bool IsApiLeagueSupported(int leagueApiId) 
{
	...
	if (leagueApiId == 45) return true; // FIFA Club World Cup
...
void CorrectUserAwards(PGconn* pg)
{
...
{
	{
		...
		//if (status == "FT")
		{
			...
			if (weekType == 0)
			{
				...
				// Update league table for both teams
				updateLeagueTable(team1Id, team1Goals, team2Goals, pointsForTeam1);
				updateLeagueTable(team2Id, team2Goals, team1Goals, pointsForTeam2);
			}
			int isSpecial = false;
