    ...
    rapidjson::Document& document)
{
    ...
    std::string sql =
        ...
        "m.status, m.is_special, m.preview, m.teaser, m.play_off, m.team1_score_90, m.team2_score_90, m.team1_score_pen, m.team2_score_pen, "
        "t1.id AS team1_id, t1.name AS team1_name, t1.short_name AS team1_short_name, "
        "t2.id AS team2_id, t2.name AS team2_name, t2.short_name AS team2_short_name, "
    ...
    for (int i = 0; i < nrows; ++i)
    {
        ...
        std::string title = PQgetvalue(ret, i, 31);
        std::string translatedTitle;
        if (isSpecial)
        {
            std::string filename = "data/special.json";
        ...
        matchObj.AddMember("special_match_stadium", rapidjson::Value(PQgetvalue(ret, i, 32), allocator), allocator);
        matchObj.AddMember("special_match_points", rapidjson::Value(PQgetvalue(ret, i, 33), allocator), allocator);
    ...
    rapidjson::Document& document)
{
    ...
    if (!ret || PQresultStatus(ret) != PGRES_TUPLES_OK)
    {
        fprintf(stderr, "Failed to fetch matches with fireball: %s", PQerrorMessage(pg));
        PQclear(ret);
        ConnectionPool::Get()->releaseConnection(pg);
        return false;
    }
    ...
    for (int i = 0; i < nrows; ++i)
    {
        ...
        // Special match translations
        std::string title = PQgetvalue(ret, i, 33);
        std::string translatedTitle;
        if (isSpecial)
        {
            std::string filename = "data/special.json";
        ...
        matchObj.AddMember("special_match_stadium", rapidjson::Value(PQgetvalue(ret, i, 34), allocator), allocator);
        matchObj.AddMember("special_match_points", rapidjson::Value(PQgetvalue(ret, i, 35), allocator), allocator);
    ...
    rapidjson::Document& document)
{
    ...
    if (!ret || PQresultStatus(ret) != PGRES_TUPLES_OK)
    {
        fprintf(stderr, "Failed to fetch matches with carrer: %s", PQerrorMessage(pg));
        PQclear(ret);
        ConnectionPool::Get()->releaseConnection(pg);
        return false;
    }
...
bool MatchesManager::GetLeagueMatchesWithEFootball(PGconn* pg,
    int userId,
    int lid,
    const std::string& season,
    const std::string& week,
    ...
    rapidjson::Document& document)
{
    ...
    if (!ret || PQresultStatus(ret) != PGRES_TUPLES_OK)
    {
        fprintf(stderr, "Failed to fetch matches with eFootball: %s", PQerrorMessage(pg));
        PQclear(ret);
        ConnectionPool::Get()->releaseConnection(pg);
        return false;
    }
    ...
    for (int i = 0; i < nrows; ++i)
    {
        ...
        // Special match translations
        std::string title = PQgetvalue(ret, i, 30);
        std::string translatedTitle;
        if (isSpecial)
        {
            std::string filename = "data/special.json";
        ...
        matchObj.AddMember("special_match_title", rapidjson::Value(PQgetvalue(ret, i, 30), allocator), allocator);
        matchObj.AddMember("special_match_stadium", rapidjson::Value(PQgetvalue(ret, i, 31), allocator), allocator);
        ...
        predictObj.AddMember("id", atoi(PQgetvalue(ret, i, 26)), allocator);
        predictObj.AddMember("team_id", atoi(PQgetvalue(ret, i, 27)), allocator);
        predictObj.AddMember("points", atoi(PQgetvalue(ret, i, 28)), allocator);
        predictObj.AddMember("status", atoi(PQgetvalue(ret, i, 29)), allocator);

        matchObj.AddMember("eFootballPredict", predictObj, allocator);
...
bool MatchesManager::GetMatchesByDateWithEFootball(PGconn* pg,
    int userId,
    long long dayStart,
    long long dayEnd,
    const std::string& lang,
...
)
{
    ...
    for (int i = 0; i < nrows; ++i)
    {
        ...
        std::string leagueName = PQgetvalue(ret, i, 30); // Assuming league_name is the last column
        std::string leagueCountry = PQgetvalue(ret, i, 31); // Assuming league_name is the last column
        int currentWeek = atol(PQgetvalue(ret, i, 32));
...
bool MatchesManager::GetMatchesLiveWithEFootball(PGconn* pg,
    int userId,
    long long currentTimeMs,
    rapidjson::Document& document
)
{
    ...
    for (int i = 0; i < nrows; ++i)
    {
        ...
        std::string specialMatchTitle = PQgetvalue(ret, i, 27);
        std::string specialMatchPoints = PQgetvalue(ret, i, 28);
        ...
        predictObj.AddMember("id", atoi(PQgetvalue(ret, i, 20)), allocator);
        predictObj.AddMember("team_id", atoi(PQgetvalue(ret, i, 21)), allocator);
        predictObj.AddMember("points", atoi(PQgetvalue(ret, i, 22)), allocator);
...
bool MatchesManager::GetMatchesUpcomingWithEFootball(PGconn* pg,
    int userId,
    long long currentTimeMs,
    rapidjson::Document& document
)
...
bool MatchesManager::GetMatchesTeamWithEFootball(PGconn* pg,
    int userId,
    long tid,
    rapidjson::Document& document
)
{
    ...
    for (int i = 0; i < nrows; ++i)
    {
        ...
        predictObj.AddMember("id", atoi(PQgetvalue(ret, i, 28)), allocator);
        predictObj.AddMember("team_id", atoi(PQgetvalue(ret, i, 29)), allocator);
        predictObj.AddMember("points", atoi(PQgetvalue(ret, i, 30)), allocator);
        predictObj.AddMember("status", atoi(PQgetvalue(ret, i, 31)), allocator);
        matchObj.AddMember("eFootballPredict", predictObj, allocator);
...
bool MatchesManager::FillTeamMatchesWithEFootball(PGconn* pg,
    int userId,
    int tid,
    rapidjson::Value& document,
    rapidjson::Document::AllocatorType& allocator,
