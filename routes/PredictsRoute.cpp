...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetUserPredict()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string tableName = "eltorneo_predicts_" + season;
       
        PGconn* pg = ConnectionPool::Get()->getConnection();
        std::string sql = "SELECT * FROM " + tableName + " WHERE user_id = "
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetUserBet()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string tableName = "beatbet_bets_" + season;
        ...
        PGconn* pg = ConnectionPool::Get()->getConnection();
        std::string sql = "select id, bet, amount, odd, status FROM " + tableName + " WHERE user_id = "
            + std::to_string(userId) + " AND match_id = "
            + matchId
            + ";";
        PGresult* ret = PQexec(pg, sql.c_str());
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetUserWorldCupPredicts()
{
    return [&](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
        std::string userId = req.get_param_value("user_id");
        std::string leagueId = "24"; // World Cup
        int page = std::stoi(req.get_param_value("page")) - 1;
        int limit = 20;
        int offset = page * limit;
        ...
        std::string sqlBase = "SELECT p.*, m.id, l.id as league_id, l.name as league_name, l.country as league_country, m.season, m.week, m.match_date, "
            ...
            "COALESCE(s.title, '') AS special_match_title, " // Fetch title from special_matches table
            "COALESCE(s.points, '') AS special_match_points " // Fetch title from special_matches table
            "FROM eltorneo_predicts_25_26 p "
            "INNER JOIN matches m ON p.match_id = m.id "
            "LEFT JOIN leagues l ON m.league = l.id "
            "LEFT JOIN teams t1 ON m.team1 = t1.id "
            "LEFT JOIN teams t2 ON m.team2 = t2.id "
        ...
        auto generateCountQuery = [&](const std::string& condition)
        {
            std::string countSql = "SELECT COUNT(*) FROM eltorneo_predicts_25_26 p INNER JOIN matches m ON p.match_id = m.id WHERE p.user_id = " + userId;
            if (leagueId != "-1")
            {
                countSql += " AND m.league = " + leagueId;
            }
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetUserBets() {
    return [&](const httplib::Request& req, httplib::Response& res) {
        ...
        auto generateBetCountQuery = [&](const std::string& condition)
        {
            std::string countSql = "SELECT COUNT(*) FROM beatbet_bets_26_27 b INNER JOIN matches m ON b.match_id = m.id WHERE b.user_id = " + userId;
            if (leagueId != "-1")
            {
                countSql += " AND m.league = " + leagueId;
            }
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetMatchPredicts()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string predictsSql = "SELECT * FROM " + tableName + " WHERE status <> 4 AND match_id = " + matchId + ";";
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetMatchBetsSummary()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        PGconn* pg = ConnectionPool::Get()->getConnection();
        std::string predictsSql = "SELECT bet FROM " + tableName + " WHERE match_id = " + matchId + ";";
        PGresult* predictsRet = PQexec(pg, predictsSql.c_str());
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetMatchPredictsTop3()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        ...
        if (season == "26/27") 
        {
            sql = "SELECT p.*, u.name, u.avatar, elu.points, "
                ...
                "COALESCE(wu.points, -1) AS world_cup_points "
                ...
                "ORDER BY elu.points" + postfix + " DESC LIMIT 20;";
        }
        else
        {
            sql = "SELECT p.*, u.name, u.avatar, u.points" + postfix + ", u.league, u.balance, "
                "COALESCE(fu.points, -1) AS fireball_points "
        ...
        if (!ret || PQresultStatus(ret) != PGRES_TUPLES_OK)
        {
            fprintf(stderr, "Failed to fetch top 20 predicts: %s", PQerrorMessage(pg));
            PQclear(ret);
            res.status = 500;  // Internal Server Error
            ConnectionPool::Get()->releaseConnection(pg);
            return;
...
void PredictsRoute::GetMatchWorldCupPredictsTop20V3(const httplib::Request& req, httplib::Response& res)
{
    ...
    std::string sql = "SELECT p.*, u.name, u.avatar, elu.points, bu.clear_balance, "
        ...
        "WHERE p.status <> 4 and p.match_id = " + matchId + " "
        "ORDER BY wu.points DESC LIMIT 20;";
    PGresult* ret = PQexec(pg, sql.c_str());
    ...
    for (int i = 0; i < nrows; ++i)
    {
        ...
        int worldCupPoints = atoi(PQgetvalue(ret, i, 21));
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetMatchWorldCupPredictsTop20V2()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
        ...
        if (season == "26_27")
        {
            GetMatchWorldCupPredictsTop20V3(req, res);
            return;
        }
        PGconn* pg = ConnectionPool::Get()->getConnection();
        // Join the predicts with users table and order by points descending, limit to 3
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetMatchBetsTop20V2()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        ...
        PGconn* pg = ConnectionPool::Get()->getConnection();
        std::string sql = "SELECT b.*, u.name, u.avatar, elu.points, bu.clear_balance, "
            ...
            "FROM beatbet_bets_" + season + " b "
           
            "JOIN users u ON b.user_id = u.id "
            "LEFT JOIN beatbet_users_" + season + " bu ON bu.user_id = u.id "
            "LEFT JOIN eltorneo_users_26_27 elu ON elu.user_id = u.id "
            "LEFT JOIN fireball_users_26_27 fu ON fu.user_id = u.id "
            "LEFT JOIN career_users_26_27 cu ON cu.user_id = u.id "
            ...
            "ORDER BY bu.clear_balance DESC LIMIT 20;";
        PGresult* ret = PQexec(pg, sql.c_str());
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetElTorneoTableV3()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
        ...
        std::string predictsTableName = "eltorneo_predicts_" + season;

        // SQL query
        std::string sql =
            "SELECT u.id, u.name, u.avatar, elu.points AS user_points, elu.league, "
            "bu.clear_balance, elu.position, COUNT(p.id) AS total_predictions, "
            "bu.league, bu.position, "
            "fu.league, fu.position, "
            "cu.league, cu.position, "
            "COALESCE(fu.points, -1) AS fireball_points, "
            ...
            "WHERE p.status != 4 AND elu.position > 0 "
            "AND elu.league = " + std::to_string(league) + " "
            
            "GROUP BY u.id, u.name, u.avatar, elu.position, elu.points, elu.league, bu.clear_balance, bu.league, bu.position, fu.points, cu.points, fu.league, fu.position, cu.league, cu.position "
            "HAVING COUNT(p.id) > 0 "
            "ORDER BY elu.position, elu.points DESC, total_predictions DESC, u.id ASC "
            "LIMIT " + std::to_string(limit) + " OFFSET " + std::to_string(offset) + ";";
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetBeatBetTableV3()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
       ...
       /* if (req.has_param("league"))
        ...
        std::string predictsTableName = "beatbet_bets_" + season;

        std::string sql =
            "SELECT u.id, u.name, u.avatar, elu.points, bu.clear_balance, "
            "elu.league, elu.position, "
            "bu.league, bu.position, "
            "fu.league, fu.position, "
            "cu.league, cu.position, "
            ...
            "GROUP BY bu.position, bu.id, u.id, u.name, u.avatar, elu.league, elu.position, elu.points, bu.league, bu.clear_balance, "
            "fu.points, cu.points, fu.league, fu.position, cu.league, cu.position "
            "HAVING COUNT(p.id) > 0 "
            "ORDER BY bu.position ASC, bu.clear_balance DESC, total_predictions DESC, bu.id ASC "
            "LIMIT " + std::to_string(limit) + " OFFSET " + std::to_string(offset) + ";";
        ...
        sql = "SELECT COUNT(*) FROM beatbet_users_" + season + " WHERE position > 0;";
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::PostBet()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        PGconn* pg = ConnectionPool::Get()->getConnection();
        std::string tableName = "beatbet_bets_26_27";
        std::string season = "26_27";
        {
            std::string sql = "select id from " + tableName + " where user_id = "
        ...
        std::string sql = "INSERT INTO " + tableName + "(user_id, match_id, bet, amount, odd, status) VALUES("
        ...
        
        {
            ...
            auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch());
            long long timestamp = ms.count();
            std::string sql = "UPDATE beatbet_users_26_27 SET balance = balance - " + std::to_string(amount) + ", last_predict_ts = " + std::to_string(timestamp) + " WHERE user_id = " + std::to_string(userId) + ";";
            PGresult* tsRet = PQexec(pg, sql.c_str());
            PQclear(tsRet);
        }
        {
            std::string sql =
                "INSERT INTO beatbet_users_26_27 (user_id, balance) VALUES (" + std::to_string(userId) + ", -" + std::to_string(amount) + ") "
                "ON CONFLICT (user_id) DO NOTHING;";
            PGresult* tsRet = PQexec(pg, sql.c_str());
            if (PQresultStatus(tsRet) != PGRES_TUPLES_OK && PQresultStatus(tsRet) != PGRES_COMMAND_OK)
            {
                printf("Failed to add beatbet user. %s\n", PQerrorMessage(pg));
            }
            PQclear(tsRet);
        }
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::PostPredict()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string sql = "INSERT INTO " + tableName + "(user_id, match_id, team1_score, team2_score, status) VALUES("
            + std::to_string(userId) + ", "
            + std::to_string(matchId) + ", "
            + std::to_string(team1) + ", "
            + std::to_string(team2) + ", "
        ...
        
        {
            ...
            auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch());
            long long timestamp = ms.count();
            std::string sql = "UPDATE eltorneo_users_26_27 SET last_predict_ts = " + std::to_string(timestamp) + " WHERE user_id = " + std::to_string(userId) + ";";
            PGresult* tsRet = PQexec(pg, sql.c_str());
            PQclear(tsRet);
        }
        {
            std::string sql =
                "INSERT INTO eltorneo_users_26_27 (user_id) VALUES (" + std::to_string(userId) + ") "
                "ON CONFLICT (user_id) DO NOTHING;";
            PGresult* tsRet = PQexec(pg, sql.c_str());
            if (PQresultStatus(tsRet) != PGRES_TUPLES_OK && PQresultStatus(tsRet) != PGRES_COMMAND_OK)
            {
                printf("Failed to add eltorneo user. %s\n", PQerrorMessage(pg));
            }
            PQclear(tsRet);
        }
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::DeleteBet()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string sql = "SELECT amount, match_id FROM beatbet_bets_26_27 WHERE user_id = " + std::to_string(userId)
            + " AND id = " + betId + ";";
        PGconn* pg = ConnectionPool::Get()->getConnection();
        PGresult* ret = PQexec(pg, sql.c_str());
        ...
        {
            sql = "UPDATE beatbet_users_26_27 set balance = balance + " + std::to_string(amount) + " WHERE user_id = " + std::to_string(userId) + ";";
            ret = PQexec(pg, sql.c_str());
            PQclear(ret);
        }

        sql = "DELETE FROM beatbet_bets_26_27 WHERE user_id = " + std::to_string(userId)
            + " AND id = " + betId + ";";
        ret = PQexec(pg, sql.c_str());
        PQclear(ret);
        ConnectionPool::Get()->releaseConnection(pg);
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::EditPredict()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        int team2 = document["team2_score"].GetInt();
        int matchId = -1;
        if (document.HasMember("match_id")) 
        {
            matchId = document["match_id"].GetInt();
        }
        std::string season = "26_27";
        PGconn* pg = ConnectionPool::Get()->getConnection();
        ...
        // Connect to the database
        std::string sql = "UPDATE " + tableName + " set team1_score = " + std::to_string(team1)
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::PostFireballPredict()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        // Connect to the database
        std::string tableName = "fireball_predicts_26_27";
        std::string usersTableName = "fireball_users_26_27";
        PGconn* pg = ConnectionPool::Get()->getConnection();
        {
            std::string sql = "select id from " + tableName + " where user_id = "
                + std::to_string(userId) + " AND match_id = " + std::to_string(matchId) + ";";
            ...
            if (n > 0)
            {
                ...
                std::string sql = "UPDATE " + tableName + " SET team_id = " + std::to_string(teamId)
        ...
        std::string sql = "INSERT INTO " + tableName + "(user_id, match_id, team_id, player_api_id, player_name, player_photo) VALUES("
            + std::to_string(userId) + ", "
            + std::to_string(matchId) + ", "
        ...
        {
            std::string sql =
                "INSERT INTO " + usersTableName + " (user_id) VALUES (" + std::to_string(userId) + ") "
                "ON CONFLICT (user_id) DO NOTHING;";
            PGresult* tsRet = PQexec(pg, sql.c_str());
            PQclear(tsRet);
        }

        {
            ...
            auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch());
            long long timestamp = ms.count();
            sql = "UPDATE " + usersTableName + " SET last_predict_ts = " + std::to_string(timestamp) + " WHERE user_id = " + std::to_string(userId) + ";";
            PGresult* tsRet = PQexec(pg, sql.c_str());
            PQclear(tsRet);
        }
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetUserFireballPredict()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string tableName = "fireball_predicts_" + season;
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetMatchFireballTop20V2()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        ...
        PGconn* pg = ConnectionPool::Get()->getConnection();
        std::string sql = "SELECT p.*, u.name, u.avatar, fu.points, elu.points, bu.clear_balance, "
            "elu.league, elu.position, "
            "bu.league, bu.position, "
            "fu.league, fu.position, "
            "cu.league, cu.position "
            "FROM fireball_predicts_" + season + " p "
            "JOIN users u ON p.user_id = u.id "
            "JOIN fireball_users_" + season + " fu ON p.user_id = fu.user_id "
            "LEFT JOIN eltorneo_users_26_27 elu ON elu.user_id = u.id "
            "LEFT JOIN beatbet_users_26_27 bu ON bu.user_id = u.id "
            "LEFT JOIN career_users_26_27 cu ON cu.user_id = u.id "
            "WHERE p.match_id = " + matchId + " "
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetFireballTableV3()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
       ...
       /* if (req.has_param("league"))
        ...
        std::string predictsTableName = "fireball_predicts_" + season;

        std::string sql = "SELECT u.id, u.name, u.avatar, fu.points, COUNT(p.id) AS total_predictions, elu.points, bu.clear_balance, "
            "elu.league, elu.position, "
            "bu.league, bu.position, "
            "fu.league, fu.position, "
            "cu.league, cu.position, "
            ...
            "FROM users u "
            "INNER JOIN " + predictsTableName + " p ON u.id = p.user_id "
            "INNER JOIN fireball_users_" + season + " fu ON u.id = fu.user_id "
            "LEFT JOIN eltorneo_users_26_27 elu ON elu.user_id = u.id "
            "LEFT JOIN beatbet_users_26_27 bu ON bu.user_id = u.id "
            "LEFT JOIN career_users_26_27 cu ON cu.user_id = u.id "
        ...
        sql = "SELECT COUNT(*) FROM fireball_users_" + season + " WHERE position > 0;";
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetUserFireballPredicts() {
    return [&](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string sql =
            ...
            "t2.id AS team2_id, t2.name AS team2_name, t2.short_name AS team2_short_name, "
            "COALESCE(s.title, '') AS special_match_title, COALESCE(s.points, '') AS special_match_points "
            "FROM fireball_predicts_26_27 fp "
        ...
        auto generateBetCountQuery = [&](const std::string& condition)
        {
            std::string countSql = "SELECT COUNT(*) FROM fireball_predicts_26_27 fp INNER JOIN matches m ON fp.match_id = m.id WHERE fp.user_id = " + userId;
            if (leagueId != "-1")
            {
                countSql += " AND m.league = " + leagueId;
            }
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetUserEFootballPredicts() {
    return [&](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
        ...
        // SQL query to fetch bets and match details
        std::string sql =
            "SELECT ep.id, ep.team_id, ep.points, ep.status, "
            "m.id AS match_id, l.id AS league_id, l.name AS league_name, l.country AS league_country, "
            "m.season, m.week, m.match_date, m.team1_score, m.team2_score, m.week_type, m.elapsed, "
            "m.team1_score_live, m.team2_score_live, m.status AS match_status, m.is_special, m.preview, "
            "m.teaser, m.play_off, "
            ...
            "t2.id AS team2_id, t2.name AS team2_name, t2.short_name AS team2_short_name, "
            "COALESCE(s.title, '') AS special_match_title, COALESCE(s.points, '') AS special_match_points "
            "FROM efootball_predicts_26_27 ep "
            "INNER JOIN matches m ON ep.match_id = m.id "
            "LEFT JOIN leagues l ON m.league = l.id "
            "LEFT JOIN teams t1 ON m.team1 = t1.id "
            "LEFT JOIN teams t2 ON m.team2 = t2.id "
            "LEFT JOIN special_matches s ON s.match_id = m.id "
        ...
        if (PQresultStatus(ret) != PGRES_TUPLES_OK)
        {
            fprintf(stderr, "Failed to fetch user eFootball predicts: %s", PQerrorMessage(pg));
            PQclear(ret);
            ConnectionPool::Get()->releaseConnection(pg);
            res.status = 500;
            return;
        ...
        for (int i = 0; i < nrows; ++i) {
            ...
            rapidjson::Value leagueObject(rapidjson::kObjectType);
            leagueObject.AddMember("id", atoi(PQgetvalue(ret, i, 5)), allocator);
            leagueObject.AddMember("name", rapidjson::StringRef(PQgetvalue(ret, i, 6)), allocator);
            leagueObject.AddMember("country", rapidjson::StringRef(PQgetvalue(ret, i, 7)), allocator);
            object.AddMember("league", leagueObject, allocator);

            object.AddMember("id", atoi(PQgetvalue(ret, i, 4)), allocator);
            object.AddMember("season", rapidjson::StringRef(PQgetvalue(ret, i, 8)), allocator);
            object.AddMember("week", atoi(PQgetvalue(ret, i, 9)), allocator);
            object.AddMember("date", (double)atoll(PQgetvalue(ret, i, 10)), allocator);
            object.AddMember("team1_score", atoi(PQgetvalue(ret, i, 11)), allocator);
            object.AddMember("team2_score", atoi(PQgetvalue(ret, i, 12)), allocator);
            object.AddMember("week_type", atoi(PQgetvalue(ret, i, 13)), allocator);
            object.AddMember("elapsed", atoi(PQgetvalue(ret, i, 14)), allocator);
            object.AddMember("team1_score_live", atoi(PQgetvalue(ret, i, 15)), allocator);
            object.AddMember("team2_score_live", atoi(PQgetvalue(ret, i, 16)), allocator);
            object.AddMember("status", rapidjson::StringRef(PQgetvalue(ret, i, 17)), allocator);
            object.AddMember("is_special", atoi(PQgetvalue(ret, i, 18)), allocator);
            object.AddMember("preview", rapidjson::StringRef(PQgetvalue(ret, i, 19)), allocator);
            object.AddMember("teaser", rapidjson::StringRef(PQgetvalue(ret, i, 20)), allocator);
            object.AddMember("playOff", atoi(PQgetvalue(ret, i, 21)), allocator);

            object.AddMember("team1_score_90", atoi(PQgetvalue(ret, i, 22)), allocator);
            object.AddMember("team2_score_90", atoi(PQgetvalue(ret, i, 23)), allocator);
            object.AddMember("team1_score_pen", atoi(PQgetvalue(ret, i, 24)), allocator);
            object.AddMember("team2_score_pen", atoi(PQgetvalue(ret, i, 25)), allocator);

            object.AddMember("special_match_title", rapidjson::Value(PQgetvalue(ret, i, 32), allocator), allocator);
            object.AddMember("special_match_points", rapidjson::Value(PQgetvalue(ret, i, 33), allocator), allocator);

            rapidjson::Value team1Object(rapidjson::kObjectType);
            team1Object.AddMember("id", atoi(PQgetvalue(ret, i, 26)), allocator);
            team1Object.AddMember("name", rapidjson::StringRef(PQgetvalue(ret, i, 27)), allocator);
            team1Object.AddMember("shortName", rapidjson::StringRef(PQgetvalue(ret, i, 28)), allocator);
            object.AddMember("team1", team1Object, allocator);

            rapidjson::Value team2Object(rapidjson::kObjectType);
            team2Object.AddMember("id", atoi(PQgetvalue(ret, i, 29)), allocator);
            team2Object.AddMember("name", rapidjson::StringRef(PQgetvalue(ret, i, 30)), allocator);
            team2Object.AddMember("shortName", rapidjson::StringRef(PQgetvalue(ret, i, 31)), allocator);
            object.AddMember("team2", team2Object, allocator);

            rapidjson::Value predictObj(rapidjson::kObjectType);
            predictObj.AddMember("id", atoi(PQgetvalue(ret, i, 0)), allocator);
            predictObj.AddMember("team_id", atoi(PQgetvalue(ret, i, 1)), allocator);
            predictObj.AddMember("points", atoi(PQgetvalue(ret, i, 2)), allocator);
            predictObj.AddMember("status", atoi(PQgetvalue(ret, i, 3)), allocator);
            object.AddMember("eFootballPredict", predictObj, allocator);
        ...
        auto generateBetCountQuery = [&](const std::string& condition)
        {
            std::string countSql = "SELECT COUNT(*) FROM efootball_predicts_26_27 ep INNER JOIN matches m ON ep.match_id = m.id WHERE ep.user_id = " + userId;
            if (leagueId != "-1")
            {
                countSql += " AND m.league = " + leagueId;
            }
        ...
        std::string countAllBetsSql = generateBetCountQuery("");  // Count all bets
        std::string countWinBetsSql = generateBetCountQuery("ep.status = 1"); // Count win bets
        std::string countLooseBetsSql = generateBetCountQuery("ep.status = 2"); // Count lose bets
        std::string countDrawBetsSql = generateBetCountQuery("ep.status = 3"); // Count lose bets
        std::string countTotalBetsSql = generateBetCountQuery("ep.status <> 0"); // Total bets that are NOT pending

        PGresult* retAllBets = PQexec(pg, countAllBetsSql.c_str());
        PGresult* retWinBets = PQexec(pg, countWinBetsSql.c_str());
        PGresult* retLooseBets = PQexec(pg, countLooseBetsSql.c_str());
        PGresult* retDrawBets = PQexec(pg, countDrawBetsSql.c_str());
        PGresult* retTotalBets = PQexec(pg, countTotalBetsSql.c_str());

        int allBets = (PQntuples(retAllBets) > 0) ? atoi(PQgetvalue(retAllBets, 0, 0)) : 0;
        int totalWinBets = (PQntuples(retWinBets) > 0) ? atoi(PQgetvalue(retWinBets, 0, 0)) : 0;
        int totalLooseBets = (PQntuples(retLooseBets) > 0) ? atoi(PQgetvalue(retLooseBets, 0, 0)) : 0;
        int totalDrawBets = (PQntuples(retDrawBets) > 0) ? atoi(PQgetvalue(retDrawBets, 0, 0)) : 0;
        int totalBets = (PQntuples(retTotalBets) > 0) ? atoi(PQgetvalue(retTotalBets, 0, 0)) : 0;
        ...
        document.AddMember("totalWinPredicts", totalWinBets, allocator);
        document.AddMember("totalLoosePredicts", totalLooseBets, allocator);
        document.AddMember("totalDrawPredicts", totalDrawBets, allocator);
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::DeleteFireballPredict()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string sql = "DELETE FROM fireball_predicts_26_27 WHERE user_id = " + std::to_string(userId)
            + " AND id = " + id + ";";
        PGconn* pg = ConnectionPool::Get()->getConnection();
        PGresult* ret = PQexec(pg, sql.c_str());
        PQclear(ret);
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::PostCareerSave()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        {
            std::string sql =
                "INSERT INTO career_users_26_27 (user_id, formation, next_transfer_ts, num_transfers) VALUES ("
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetCareerTableV2()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string sql = "SELECT u.id, u.name, u.avatar, elu.points, bu.clear_balance, elu.points, cu.points, "
            "elu.league, elu.position, "
            "bu.league, bu.position, "
            "fu.league, fu.position, "
            "cu.league, cu.position "
            "FROM users u "
            "INNER JOIN career_users_" + season + " cu ON u.id = cu.user_id "
            "LEFT JOIN eltorneo_users_26_27 elu ON elu.user_id = u.id "
            "LEFT JOIN beatbet_users_26_27 bu ON bu.user_id = u.id "
            "LEFT JOIN fireball_users_26_27 fu ON fu.user_id = u.id "
            "WHERE cu.league = " + std::to_string(league) + " "
            " GROUP BY u.id, u.name, u.avatar, cu.points, fu.points, fu.league, fu.position, cu.points, cu.league, cu.position, elu.points, elu.league, elu.position, bu.clear_balance, bu.league, bu.position "
            "ORDER BY cu.points DESC, u.id ASC "
            "LIMIT " + std::to_string(limit) + " OFFSET " + std::to_string(offset) + ";";
        ...
        sql = "SELECT COUNT(*) FROM career_users_" + season + " WHERE position > 0;";
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetUserCareerPredicts() {
    return [&](const httplib::Request& req, httplib::Response& res) {
        ...
        auto generateBetCountQuery = [&](const std::string& condition)
        {
            std::string countSql = "SELECT COUNT(*) FROM career_predicts_26_27 cp INNER JOIN matches m ON cp.match_id = m.id WHERE cp.user_id = " + userId;
            if (leagueId != "-1")
            {
                countSql += " AND m.league = " + leagueId;
            }
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::PostEFootballPredict()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        ...
        PGconn* pg = ConnectionPool::Get()->getConnection();
        std::string season = "26_27";
        std::string tableName = "efootball_predicts_" + season;
        std::string usersTableName = "efootball_users_" + season;

        /*{
            std::string sql = "SELECT season FROM matches WHERE id = " + std::to_string(matchId) + ";";
            PGresult* ret = PQexec(pg, sql.c_str());
            season = PQgetvalue(ret, 0, 0);
            PQclear(ret);

            if (season == "26/27") 
            {
                tableName += "_26_27";
                usersTableName += "_26_27";
            }
        }*/

        std::string selectSQL = "select id from " + tableName + " where user_id = "
            + std::to_string(userId) + " AND match_id = " + std::to_string(matchId) + ";";
        PGresult* selectRet = PQexec(pg, selectSQL.c_str());
        if (!selectRet || PQresultStatus(selectRet) != PGRES_TUPLES_OK && PQresultStatus(selectRet) != PGRES_COMMAND_OK)
        {
        ...
        if (n > 0)
        {
            ...
            std::string updateSQL = "UPDATE " + tableName + " set team_id = " + std::to_string(teamId) 
                + " WHERE id = " + std::to_string(predictId) + ";";
        ...
        if (!updated)
        {
            std::string sql = "INSERT INTO " + tableName + "(user_id, match_id, team_id) VALUES("
                + std::to_string(userId) + ", "
                + std::to_string(matchId) + ", "
                + std::to_string(teamId) + " "
                + ") RETURNING id";  // Return the inserted ID
...

        {
            ...
            auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch());
            long long timestamp = ms.count();
            std::string sql = "UPDATE " + usersTableName + " SET last_predict_ts = " + std::to_string(timestamp) + " WHERE user_id = " + std::to_string(userId) + ";";
            PGresult* tsRet = PQexec(pg, sql.c_str());
            PQclear(tsRet);
        }

        {
            ...
            if (PQresultStatus(tsRet) != PGRES_TUPLES_OK && PQresultStatus(tsRet) != PGRES_COMMAND_OK)
            {
                printf("Failed to add eFootball user. %s\n", PQerrorMessage(pg));
            }
            PQclear(tsRet);
        }
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetEFootballTableV2()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
        ...
        std::string predictsTableName = "efootball_predicts_" + season; 

        std::string sql = "SELECT u.id, u.name, u.avatar, COUNT(p.id) AS total_predictions, elu.points, bu.clear_balance, "
            "elu.league, elu.position, "
            "bu.league, bu.position, "
            ...
            "FROM users u "
            "INNER JOIN " + predictsTableName + " p ON u.id = p.user_id "
            "INNER JOIN efootball_users_" + season + " eu ON u.id = eu.user_id "
            "LEFT JOIN eltorneo_users_26_27 elu ON elu.user_id = u.id "
            "LEFT JOIN beatbet_users_26_27 bu ON bu.user_id = u.id "
            "LEFT JOIN fireball_users_26_27 fu ON fu.user_id = u.id "
            "LEFT JOIN career_users_26_27 cu ON cu.user_id = u.id "
            ...
            "WHERE eu.position > 0 "
            "AND eu.league = " + std::to_string(league) + " "
            
            " GROUP BY u.id, u.name, u.avatar, eu.points, eu.league, eu.position, elu.position, elu.points, elu.league, bu.position, bu.clear_balance, bu.league, cu.points, fu.league, fu.position, cu.league, cu.position, fu.points "
            "HAVING COUNT(p.id) > 0 "
        ...
        if (PQresultStatus(ret) != PGRES_TUPLES_OK)
        {
            fprintf(stderr, "Failed to fetch eFootball table: %s", PQerrorMessage(pg));
            PQclear(ret);
            ConnectionPool::Get()->releaseConnection(pg);
            res.status = 500;
            return;
        ...
        for (int i = 0; i < nrows; ++i)
        {
            ...
            object.AddMember("name", rapidjson::Value(PQgetvalue(ret, i, 1), allocator), allocator);
            object.AddMember("avatar", rapidjson::Value(PQgetvalue(ret, i, 2), allocator), allocator);
            object.AddMember("totalPredictions", atoi(PQgetvalue(ret, i, 3)), allocator);
            object.AddMember("predictions", atoi(PQgetvalue(ret, i, 4)), allocator);
            object.AddMember("balance", atof(PQgetvalue(ret, i, 5)), allocator);
        ...
        sql = "SELECT COUNT(*) FROM efootball_users_" + season + " WHERE position > 0;";
        PGresult* totalRet = PQexec(pg, sql.c_str());
        int totalUsers = atoi(PQgetvalue(totalRet, 0, 0));
        document.AddMember("numUsers", totalUsers, allocator);
        PQclear(totalRet);
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetMatchEFootballSummary()
{
    return [](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
        ...
        for (int i = 0; i < nrows; ++i)
        {
            int teamId = atoi(PQgetvalue(predictsRet, i, 0));
            int team1 = atoi(PQgetvalue(predictsRet, i, 1));
            int team2 = atoi(PQgetvalue(predictsRet, i, 2));

            if (teamId == team1) ++numTeam1;
            if (teamId == team2) ++numTeam2;
        }

        document.AddMember("numTeam1", numTeam1, allocator);
        document.AddMember("numTeam2", numTeam2, allocator);
        document.AddMember("numPredicts", nrows, allocator);
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetMatchEFootballTop20V2()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
...
std::function<void(const httplib::Request&, httplib::Response&)> PredictsRoute::GetWorldCupTableV2()
{
    return [this](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string predictsTableName = "predicts";
        std::string pointsColName = "wu.points";
        ...
        std::string sql = "SELECT u.id, u.name, u.avatar, COUNT(p.id) AS total_predictions, u.points, u.clear_balance, "
            ...
            "INNER JOIN " + predictsTableName + " p ON u.id = p.user_id "
            "INNER JOIN matches m ON p.match_id = m.id "
            "INNER JOIN world_cup_users wu ON u.id = wu.user_id "
            "LEFT JOIN efootball_users eu ON u.id = eu.user_id "
            "LEFT JOIN fireball_users fu ON fu.user_id = u.id "
            "LEFT JOIN career_users cu ON cu.user_id = u.id "
            "WHERE wu.position > 0 "
            "AND m.league = 24 "
            " GROUP BY u.id, u.name, u.avatar, " + pointsColName + ", wu.position, wu.points, eu.league, eu.position, eu.points, fu.league, fu.position, cu.league, cu.position, fu.points, cu.points "
            "HAVING COUNT(p.id) > 0 "
            "ORDER BY wu.position ASC, total_predictions DESC, u.id ASC "
            "LIMIT " + std::to_string(limit) + " OFFSET " + std::to_string(offset) + ";";
        ...
        for (int i = 0; i < nrows; ++i)
        {
            ...
            int worldCupPosition = atoi(PQgetvalue(ret, i, 19));
            object.AddMember("worldCupPosition", worldCupPosition, allocator);
        ...
        sql =
            ...
            "    INNER JOIN " + predictsTableName + " p ON u.id = p.user_id "
            "    INNER JOIN world_cup_users wu ON u.id = wu.user_id "
            "    WHERE wu.last_predict_ts >= " + std::to_string(timestamp - ten_days_ms) + " "
            "    GROUP BY u.id "
            "    HAVING COUNT(p.id) > 0 "
            ") AS subquery;";
        PGresult* totalRet = PQexec(pg, sql.c_str());
